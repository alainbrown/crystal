...
- Model **weights are data**, fetched at runtime from the HF Hub; they are not bundled. The extension declares **no `host_permissions`** — the HF Hub and its CDN/xet hosts serve reflective CORS (`access-control-allow-origin` echoes the request origin) across the full redirect chain, so the service worker's anonymous GETs succeed as plain CORS requests. This keeps the install prompt free of host warnings; the tradeoff is that the download would break if HF ever stopped reflecting CORS. The e2e journey is the only test that exercises this real fetch.
